    ...
    def generate_strm_files(
        ...
        for path in media_paths:
            ...
            try:
                ...
                # 必须从 payload 传入；同时强制用 list_method="list"（旧 /file/list 端点，
                # 对 driveId=0 接受，默认的 list_new 端点会报 driveId is required）。
                for item in iterdir(
                    client=self.client,
...
class P123StrmSelfuse(_PluginBase):
    ...
    def init_plugin(self, config: dict = None):
        ...
        try:
            ...
            logger.info(f"【插件初始化】user_info 原始响应: {uid_resp}")
            check_response(uid_resp)
            data = uid_resp.get("data") or uid_resp
            self._uid = (
                data.get("UID")
                or data.get("userId")
                or data.get("user_id")
    ...
    def play(
        ...
        try:
            ...
            logger.info(
                f"【播放跳转】payload={payload} _uid={self._uid!r} "
                f"file_id={file_id} etag={etag} size={size}"
            )
            ...
            logger.info(f"【播放跳转】download_info 原始响应: {resp}")
            check_response(resp)
            ...
            if file_id:
                ...
                try:
                    ...
                    if not parent_file_id:
                        ...
                        logger.warning("【302跳转服务】降级为根目录(parentFileId=0)秒传，文件将落在网盘根目录而非'我的秒传'")
                        parent_file_id = 0
